package org.instalk.cloud.instalksocialservice.friendship.service;
...
import org.instalk.cloud.instalksocialservice.friendship.mapper.FriendshipMapper;
...
public class FriendshipService {
    ...
    public Result sendFriendshipRequest(Long id) {
        ...
        if (myId == id) return Result.error("不能添加自己为好�?);
        ...
        if (friendship != null){
            if (friendship.getStatus().equals("PENDING")) return Result.error("好友申请已存�?);
            else if (friendship.getStatus().equals("BLOCKED")) return Result.error("你已被拉入黑名单或对方被你拉入了黑名�?);
            else return Result.error("已经是好�?);
        }
    ...
    public Result<FriendVO> acceptFriendshipRequest(Long id) {
        ...
        Long id2 = Math.max(myId,id);
        Friendship friendship = friendshipMapper.selectByUserId1AndUserId2(id1,id2);
        if (friendship == null) return Result.error("好友申请不存�?);
        else{
            if (friendship.getRequesterId()==myId) return Result.error("操作失败");
            if (friendship.getStatus().equals("BLOCKED")) return Result.error("你已被拉入黑名单或对方被你拉入了黑名�?);
            else if (friendship.getStatus().equals("ACCEPTED")) return Result.error("已经是好�?);
        }
    ...
    public Result rejectFriendshipRequest(Long id) {
        ...
        if (friendship == null) return Result.error("好友申请不存�?);
            ...
            if (friendship.getRequesterId()==myId) return Result.error("操作失败");
            if (friendship.getStatus().equals("ACCEPTED")) return Result.error("已经是好�?);
            else if (friendship.getStatus().equals("BLOCKED")) return Result.error("你已被拉入黑名单或对方被你拉入了黑名�?);
        }
    ...
    public Result deleteFriendship(Long id) {
        ...
        Long id2 = Math.max(myId,id);
        Friendship friendship = friendshipMapper.selectByUserId1AndUserId2(id1,id2);
        if (friendship == null) return Result.error("好友不存�?);
        else{
            if (friendship.getStatus().equals("PENDING")) return Result.error("好友不存�?);
            else if (friendship.getStatus().equals("BLOCKED")) return Result.error("你已被拉入黑名单或对方被你拉入了黑名�?);
        }
