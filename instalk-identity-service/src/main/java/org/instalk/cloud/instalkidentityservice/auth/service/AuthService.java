package org.instalk.cloud.instalkidentityservice.auth.service;
...
public class AuthService {
    ...
    public Result register(RegisterDTO registerDTO) {
        if (registerDTO.getUsername() == null) return Result.error("用户名不能为�?);
        ...
        if (userFeignClient.getUserByEmail(registerDTO.getEmail()) != null) return Result.error("邮箱已存�?);
        if (!captchaUtil.verifyCaptcha(registerDTO.getEmail(),registerDTO.getCaptcha())) return Result.error("验证码错�?);
    ...
    public Result sendCaptcha(String email) {
        ...
        if (userFeignClient.getUserByEmail(email) != null) return Result.error("邮箱已存�?);
